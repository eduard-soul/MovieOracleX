...
async def get_users_from_movie_members(client: aiohttp.ClientSession, movie_slug: str, sem: asyncio.Semaphore, add_user: callable, add_less_25_movie: callable) -> None:
    async with sem:
...
# Main execution
async def main():
    # *** Added: Initialize set and lock to track movies to remove ***
    less_25_movies = set()
    ...
    # Open CSV files for appending and set up writers
    with open(csv_file, 'a', newline='') as f, open('less_25_film_ids.csv', 'a', newline='') as f_less:
        ...
        # *** Modified: Add movie to less_25_movies set ***
        ...
        async with aiohttp.ClientSession(connector=http_conn, timeout=timeout) as client:
            ...
            movie_tasks = [
                asyncio.create_task(get_users_from_movie_members(client, film_id, sem, add_user, add_less_25_movie))
    ...
    # *** Added: Update unique_film_ids.csv by removing movies with less than 25 users ***
